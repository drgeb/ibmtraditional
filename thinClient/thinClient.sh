...
WAS_HOME="/opt/IBM/WebSphere/ThinClient"
USER_INSTALL_ROOT="/opt/IBM/WebSphere/ThinClient"
...
#wsadminTraceString=-Dcom.ibm.ws.scripting.traceString=
...
CP="${WAS_HOME}/properties:${WAS_HOME}/lib/jython/jython.jar:${WAS_HOME}/lib/com.ibm.ws.admin.client_8.5.0.jar:${WAS_HOME}/lib/com.ibm.ws.security.crypto.jar"
...
CMDLINE="${JAVA_EXE} ${PERF_JVM_OPTIONS} ${EXTRA_X_ARGS} -Dws.ext.dirs=$WAS_EXT_DIRS ${WAS_LOGGING} ${javaOptions} ${CONSOLE_ENCODING} ${WAS_DEBUG} ${CLIENTSOAP} ${JAASSOAP} ${CLIENTSAS} ${CLIENTSSL} ${CLIENTIPC} ${WSADMIN_PROPERTIES_PROP} ${WORKSPACE_PROPERTIES} -Duser.install.root=${USER_INSTALL_ROOT} -Dwas.install.root=${WAS_HOME} -Dcom.ibm.websphere.thinclient=true ${wsadminTraceFile} ${wsadminTraceString} ${wsadminHost} ${wsadminConnType} ${wsadminPort} ${wsadminLang} -classpath ${CP} ${SHELL} ${nonJavaOption[@]}"
